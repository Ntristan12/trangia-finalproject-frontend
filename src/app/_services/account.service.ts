...
async function register(params: any, origin: any) {
  if (await db.Account.findOne({ where: { email: params.email } })) {
    try { await sendAlreadyRegisteredEmail(params.email, origin); } catch(e) { console.error("[register] Email send failed:", e); } return;
  }
  ...
  try { await sendVerificationEmail(account, origin); } catch(e) { console.error("[register] Email send failed:", e); }
}
